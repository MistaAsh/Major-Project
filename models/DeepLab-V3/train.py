...
from utils import (
    ...
    multiscale_single_validate,
    save_load_means,
    subtract_channel_means,
    validation_demo,
...
def train(
    ...
    train_dataset = Dataset(
        dataset_filename=trainset_filename,
    ...
    valid_dataset = Dataset(
        dataset_filename=valset_filename,
        images_dir=images_dir,
        labels_dir=labels_dir,
        image_extension=".jpg",
        label_extension=".png",
    ...
    for i in range(num_epochs):
        ...
        for _ in trange(
            np.ceil(train_iterator.dataset_size / minibatch_size).astype(int)
        ):
            images, labels = train_iterator.next_minibatch()
            balanced_weight_decay = (
                weight_decay * sum(labels != ignore_label) / labels.size
            )
        ...
        for _ in trange(
            np.ceil(train_augmented_iterator.dataset_size / minibatch_size).astype(int)
        ):
        ...
        train_loss_ave = train_loss_total / (
            train_iterator.dataset_size + train_augmented_iterator.dataset_size
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train DeepLab V3 for image semantic segmantation."
    )

    network_backbone_default = "resnet_101"
    pre_trained_model_default = "data/models/pretrained/resnet_101/resnet_v2_101.ckpt"
    trainset_filename_default = (
        "data/datasets/VOCdevkit/VOC2012/ImageSets/Segmentation/train.txt"
    )
    valset_filename_default = (
        "data/datasets/VOCdevkit/VOC2012/ImageSets/Segmentation/val.txt"
    )
    ...
    model_dir_default = "data/models/deeplab/{}_voc2012/".format(
        network_backbone_default
    )
    ...
    parser.add_argument(
        "--network_backbone",
        type=str,
        help="Network backbones: resnet_50, resnet_101, mobilenet_1.0_224. Default: resnet_101",
        default=network_backbone_default,
    )
    parser.add_argument(
        "--pre_trained_model",
        type=str,
        help="Pretrained model directory",
        default=pre_trained_model_default,
    )
    parser.add_argument(
        "--trainset_filename",
        type=str,
        help="Train dataset filename",
        default=trainset_filename_default,
    )
    parser.add_argument(
        "--valset_filename",
        type=str,
        help="Validation dataset filename",
        default=valset_filename_default,
    )
    parser.add_argument(
        "--images_dir", type=str, help="Images directory", default=images_dir_default
    )
    parser.add_argument(
        "--labels_dir", type=str, help="Labels directory", default=labels_dir_default
    )
    parser.add_argument(
        "--trainset_augmented_filename",
        type=str,
        help="Train augmented dataset filename",
        default=trainset_augmented_filename_default,
    )
    parser.add_argument(
        "--images_augmented_dir",
        type=str,
        help="Images augmented directory",
        default=images_augmented_dir_default,
    )
    parser.add_argument(
        "--labels_augmented_dir",
        type=str,
        help="Labels augmented directory",
        default=labels_augmented_dir_default,
    )
    parser.add_argument(
        "--model_dir",
        type=str,
        help="Trained model saving directory",
        default=model_dir_default,
    )
    parser.add_argument(
        "--log_dir", type=str, help="TensorBoard log directory", default=log_dir_default
    )
    parser.add_argument(
        "--random_seed",
        type=int,
