        ...
        with open(self.dataset_filename, "r") as file:
            for line in file:
                ...
                image_filename = os.path.join(
                    self.images_dir, filename + self.image_extension
                )
                label_filename = os.path.join(
                    self.labels_dir, filename + self.label_extension
                )
...
class Iterator(object):
    def __init__(
        ...
        self.image_filenames, self.label_filenames = self.read_dataset(
            dataset=dataset, scramble=scramble, random_seed=random_seed
        )
    ...
    def next_minibatch(self):
        image_filenames_minibatch = self.image_filenames[
            self.current_index : self.current_index + self.minibatch_size
        ]
        label_filenames_minibatch = self.label_filenames[
            self.current_index : self.current_index + self.minibatch_size
        ]
        ...
        results = Parallel(n_jobs=self.num_jobs)(
            delayed(self.process_func)(image_filename, label_filename)
            for image_filename, label_filename in zip(
                image_filenames_minibatch, label_filenames_minibatch
            )
        )
...
def resize_image_and_label(image, label, output_size):
    ...
    label_resized = cv2.resize(
        label, (output_size[1], output_size[0]), interpolation=cv2.INTER_NEAREST
    )

    return image_resized, label_resized


def pad_image_and_label(
    image, label, top, bottom, left, right, pixel_value=0, label_value=255
):
    """
    ...
    image_padded = cv2.copyMakeBorder(
        image, top, bottom, left, right, cv2.BORDER_CONSTANT, value=pixel_value
    )
    label_padded = cv2.copyMakeBorder(
        label, top, bottom, left, right, cv2.BORDER_CONSTANT, value=label_value
    )
...
def random_crop(image, label, output_size):
    assert (
        image.shape[0] >= output_size[0] and image.shape[1] >= output_size[1]
    ), "image size smaller than the desired output size."
...
def image_augmentaion(
    image, label, output_size, min_scale_factor=0.5, max_scale_factor=2.0
):
    ...
    image, label = resize_image_and_label(
        image=image, label=label, output_size=rescaled_size
    )
    ...
    image, label = pad_image_and_label(
        image=image,
        label=label,
        top=vertical_pad_up,
        bottom=vertical_pad_down,
        left=horizonal_pad_left,
        right=horizonal_pad_right,
        pixel_value=0,
        label_value=255,
    )
...
def validation_demo(images, labels, predictions, demo_dir, batch_no):
    ...
    for i in range(len(images)):
        cv2.imwrite(
            os.path.join(demo_dir, "image_{}_{}.jpg".format(batch_no, i)), images[i]
        )
        save_annotation(
            label=labels[i],
            filename=os.path.join(
                demo_dir, "image_{}_{}_label.png".format(batch_no, i)
            ),
            add_colormap=True,
        )
        save_annotation(
            label=predictions[i],
            filename=os.path.join(
                demo_dir, "image_{}_{}_prediction.png".format(batch_no, i)
            ),
            add_colormap=True,
        )
...
def validation_single_demo(image, label, prediction, demo_dir, val_no):
    ...
    save_annotation(
        label=label,
        filename=os.path.join(demo_dir, "image_{}_label.png".format(val_no)),
        add_colormap=True,
    )
    save_annotation(
        label=prediction,
        filename=os.path.join(demo_dir, "image_{}_prediction.png".format(val_no)),
        add_colormap=True,
    )
...
def mean_intersection_over_union(num_pixels_union, num_pixels_intersection):
    ...
    mean_iou = np.mean(
        num_pixels_intersection[valid_classes] / num_pixels_union[valid_classes]
    )
...
def multiscale_single_test(image, input_scales, predictor):
    ...
    for input_scale in input_scales:
        ...
        image_scaled = cv2.resize(
            image,
            (image_width_scaled, image_height_scaled),
            interpolation=cv2.INTER_LINEAR,
        )
        output = predictor(
            inputs=[image_scaled],
            target_height=image_height_raw,
            target_width=image_width_raw,
        )[0]
...
if __name__ == "__main__":
    ...
    train_dataset = Dataset(
        dataset_filename="data/datasets/VOCdevkit/VOC2012/ImageSets/Segmentation/train.txt",
    ...
    voc2012_preprocessor = DataPreprocessor(
        channel_means=channel_means, output_size=[513, 513], max_scale_factor=1.5
    )
