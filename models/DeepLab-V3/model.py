...
class DeepLab(object):
    ...
    def backbone_initializer(self, base_architecture):
        with tf.variable_scope("backbone"):
            if base_architecture == "vgg16":
                features = Vgg16(
                    self.inputs, self.weight_decay, self.batch_norm_momentum
                )
    ...
    def model_initializer(self):
        pools = atrous_spatial_pyramid_pooling(
            inputs=self.feature_map, filters=256, regularizer=self.regularizer
        )
        logits = tf.layers.conv2d(
            inputs=pools, filters=self.num_classes, kernel_size=(1, 1), name="logits"
        )
    ...
    def loss_initializer(self):
        ...
        onehot_labels = tf.one_hot(
            indices=labels_linear, depth=self.num_classes, on_value=1.0, off_value=0.0
        )

        loss = tf.losses.softmax_cross_entropy(
            onehot_labels=onehot_labels,
            logits=tf.reshape(self.outputs, shape=[-1, self.num_classes]),
    ...
    def train(
        self, inputs, labels, target_height, target_width, learning_rate, weight_decay
    ):
    ...
    def validate(self, inputs, labels, target_height, target_width):
        outputs, valid_loss, summaries = self.sess.run(
            [self.outputs, self.loss, self.valid_summaries],
            feed_dict={
                self.inputs: inputs,
                self.labels: labels,
                self.target_height: target_height,
    ...
    def initialize_backbone_from_pretrained_weights(self, path_to_pretrained_weights):
        ...
        tf.train.init_from_checkpoint(
            ...
            {
                v.name[len(valid_prefix) :].split(":")[0]: v
                for v in variables_to_restore
                if v.name.startswith(valid_prefix)
            },
        )
...
if __name__ == "__main__":
    deeplab = DeepLab(
        "resnet_101",
        pre_trained_model="data/models/pretrained/resnet_101/resnet_v2_101.ckpt",
    )
